...
const NIVEL_ATIVIDADE_OPCOES = [
  'Sedentário',
  'Levemente active', // Corrigido para 'Levemente ativo'
  'Levemente ativo',
  'Moderadamente ativo',
  'Muito ativo',
  'Extremamente ativo'
...
const EditarPaciente: React.FC = () => {
    ...
    async function loadPatientData() {
      ...
      try {
        ...
        if (data) {
          ...
          setSexo(data.sexo || '');
          setPesoInicial(data.peso_inicial ? String(data.peso_inicial) : '');
          
          // Converte metros para centímetros ao carregar para o formulário
          setAltura(data.altura ? String(Math.round(data.altura * 100)) : '');
          
          setAtividadeFisica(data.atividade_fisica || false);
          setAtividadeFisicaDescricao(data.atividade_fisica_descricao || '');
          setNivelAtividade(data.nivel_atividade || '');
          setRefeicoesPorDia(data.refeicoes_por_dia ? String(data.refeicoes_por_dia) : '4');
          setHorarioAcorda(data.horario_acorda || '');
          setHorarioDorme(data.horario_dorme || '');
          setLitrosAgua(data.litros_agua ? String(data.litros_agua) : '');
          setObjetivos(data.objetivos as string[] || []);
          setObjetivoTexto(data.objetivo_texto || '');
          setPatologias(data.patologias as string[] || []);
          setRestricoesAlimentares(data.restricoes_alimentares as string[] || []);
          setAlergias(data.alergias as string[] || []);
  ...
  // Função para converter número digitado em string de hora
  ...
  // Adicionar patologia personalizada
  ...
  // Adicionar restrição personalizada
  ...
  // Adicionar alergia personalizada
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao atualizar paciente no banco de dados.');
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '900px', margin: '0 auto' }} className="animate-fade-in">
      ...
      <header style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: 'var(--text-secondary)' }}>Atualize as informações cadastrais e clínicas do paciente.</p>
      </header>
